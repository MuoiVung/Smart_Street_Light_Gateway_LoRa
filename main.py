...
def process_lora_data(data):
    ...
    if dev_id in config.DEVICE_MAP:
        ...
        if "auto_mode" in data: device_states[dev_id]["auto_mode"] = data["auto_mode"]
        if "ledBrightness" in data: device_states[dev_id]["led_brightness"] = data["ledBrightness"]
        if "yellow_color" in data: device_states[dev_id]["yellow_color"] = data["yellow_color"]
    ...
    try:
        ...
        if target_id != 0:
            ...
            if method == "setAutoMode":
                ...
                if lora: lora.send_command(target_id, "AUTO", 1 if params else 0)
                ...
                if lora: lora.send_command(target_id, "DIM", int(params))
...
if __name__ == "__main__":
    try:
        ...
        print("Exit.")
        if hasattr(lora, 'close'): lora.close() # Dừng thread giả lập nếu có
        BOARD.teardown()
        client.disconnect()